cd 'D:\吵雯的文件夹\学习与工作\工作\Shared with me\qwcwork\framerge\revise\macmini'
clear
...
% letter
ylim = get(gca, 'ylim');  % Get the current y-axis limits
text(0, ylim(2)+1, 'c', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center','FontSize',18)
% Legend
...
set(label3, 'position',[2.5000  -9   -1.0000])
% Color
...
% letter
ylim = get(gca, 'ylim');  % Get the current y-axis limits
text(-0.2, ylim(2)+5, 'f', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center','FontSize',18)
% Legend
...
% print(['macmini','.eps'],'-depsc','-r600');% save eps with 600dpi
% print(gcf, 'macmini', '-dsvg'); % save svg