...
syntax anything, frame(string) [Linkvar(name)    /*keep linkvar as specified name
	...
	if ("`subcmd'"=="1:m" ) {
        ...
        if `"`linkvar'"' == ""{
            ...
            frame `frame' : qui drop rownum_usingdta
        } 
		//exit
	}
	else if ("`subcmd'"=="m:m" ){
...
program define frlink_genmm, rclass 
	...
	*/The following codes are copyed from Stata oficial command frlink_gen*/
	...
	char `mvar'[frlink_mtyp]   m:1
	char `mvar'[frlink_fname1] `frame1'
	char `mvar'[frlink_fname2] `frame2'
	char `mvar'[frlink_vl1]    rownum_usingdta
	char `mvar'[frlink_vl2]    rownum_usingdta
	char `mvar'[frlink_vl2p]   rownum_usingdta
...
****************** My modefied codes***********************************
	...
	frame `frame2' {
		...
		sort `varlist2p'
		qui gen rownum_usingdta = _n
		qui bys `varlist2p' (rownum_usingdta): gen rowN_usingdta = (_n==_N)
        tempname _temp_
	    qui frame put `varlist2p' rownum_usingdta rowN_usingdta if rowN_usingdta , into(`_temp_')
		qui drop rowN_usingdta
	}

	   frame `_temp_'{
		qui replace rowN_usingdta = rownum_usingdta
		qui replace rowN_usingdta = rownum_usingdta - rownum_usingdta[_n-1] if _n>1
		qui gen row1_usingdta = rownum_usingdta - rowN_usingdta +1 
	   }
	   // obtain row1_u rowN_u from `_temp_'
		qui frlink m:1 `varlist', frame(`_temp_' `varlist2p')
		qui frget row1_usingdta rowN_usingdta, from(`_temp_') 
		qui frame drop `_temp_'
		cap drop `_temp_'
		// replicate the master data by rowN_u times
		tempvar gidi
		qui bys `varlist': gen `gidi' = _n
		qui expand rowN_usingdta
		//generate a rownum identifier for each obs in the master data
		qui bys `varlist' `gidi': gen rownum_usingdta = row1_usingdta + _n -1 
		qui drop row1_usingdta 
		qui drop rowN_usingdta
		...
		frame `frame2': qui _datasignature rownum_usingdta, nonames
		char `mvar'[frlink_sig2] `r(datasignature)'
		qui replace `mvar' = rownum_usingdta
		qui compress `mvar'
		if "`generate'" != "" {
			rename `mvar' `generate'
		}
		else rename `mvar' `frame2'
		qui count if missing(rownum_usingdta)
