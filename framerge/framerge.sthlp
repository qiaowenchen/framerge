...
{p2colset 1 15 17 2}{...}
{p2col:{bf:[D] framerge} {hline 2}}Link frames and copy variables from the using frames based on multiple match relationships {p_end}
...
[{cmdab:l:inkvar(}{help framerge##linkname:{it:linkname}}{cmd:)}{break}
...
{help framerge##linkname:{it:linkname}}
...
{help framerge##linkname:{it:linkname}}
[{cmd:,} {cmd:frame(}{help frlink##frame3:{it:frame3}}{cmd:)}]
...
frame, which we will call {it:frame1}.
{p_end}
...
{pmore}
    ...
    {cmd:get()} option always omits the underscore and match variables.
...
{title:Note}
{phang}
{cmd: framerge m:m} operation establishes a {cmd:joinby} relationship, in contrast to the relationship generated by the {cmd: merge m:m} operation. framerge 1:1 and framerge m:1 are aliases of frlink 1:1 
and frlink m:1, respectively. framerge also supports subcommands {cmd: dir}, {cmd: describe}, {cmd: rebuild}, and drop which are subcommands of frlink, respectively.

{marker Cautions}{...}
{title:Cautions}
{phang}
{cmd:framerge 1:m} and {cmd:framerge m:m} would create a variable rownum_usingdta. 
If this variable has existed in the current data or the using data, 
the command would throw an error. 

{phang}
{cmd:linkvar()} shold be specified when {cmd:get()} is not specifed.
